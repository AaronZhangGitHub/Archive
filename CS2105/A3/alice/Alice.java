...
class Alice {  // Alice is a TCP client
    ...
    public Alice(String ipStr, String portStr) {
        ...
        try {
            welcomeSocket = new Socket(ipStr, portNum);
        } catch (UnknownHostException e) {
            System.out.println("Can not connect to host: "+ipStr);
            e.printStackTrace();
        } catch (IOException e) {
            System.out.println("Input/Output issue connecting to host: "+ipStr);
            e.printStackTrace();
        }
        
        //Get input and output stream from Bob
        try {
            this.toBob = new ObjectOutputStream(welcomeSocket.getOutputStream());
            this.fromBob = new ObjectInputStream(welcomeSocket.getInputStream());
        } catch (IOException e) {
            System.out.println("Error recieving input/output stream");
            e.printStackTrace();
        }
        ...
        try {
            ...
            System.out.println("Error with closing TCP connection");
            e.printStackTrace();
        }
    }
    ...
    public void sendSessionKey() {
        try {
            ...
            System.out.println("Error with sending Bob the session key");
            e.printStackTrace();
        }
    }
    ...
    public void receiveMessages() {
        //Create local file and writer to file
        File messageFile = new File(System.getProperty("user.dir")+'/'+this.MESSAGE_FILE);
        PrintWriter pw = null;
        try {
            messageFile.createNewFile();
            pw = new PrintWriter(messageFile);
        } catch (IOException e1) {
            System.out.println("Problem with creating file/creating writer to file");
            e1.printStackTrace();
        }
        while(true){
            try {
                //receive message
                SealedObject receivedObj = (SealedObject) this.fromBob.readObject();
                
                //decrypt message and write to a file
                pw.write(this.crypto.decryptMsg(receivedObj)+'\n');
    ...
    class Crypto {
        ...
        public void initSessionKey() {
            ...
            try {
                ...
                System.out.println("No AES algorithim exists");
                e.printStackTrace();
            }
            kg.init(128);
        ...
        public SealedObject getSessionKey() {
            ...
            try{
                ...
                System.out.println("No such algorithim");
                e.printStackTrace();
