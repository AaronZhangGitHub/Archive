...
public class Adventure {
	...
	public Adventure(Difficulty mode, Squad squad) {
		observers = new ArrayList<TravelObserver>();
		this.mode = mode;
		this.squad = squad;
	...
	public void travel(City next_city) {
		...
		while (distance > 0) {
			NotifyObservers(distance, next_city);
	...
	public void NotifyObservers(int x, City destination){
		for(int total_size = observers.size();total_size>0;total_size--){
			observers.get(total_size-1).travelUpdate(this, x, destination);
		}
	}
